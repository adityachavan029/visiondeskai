          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div
                ...
                onDrop={handleDrop}
                className="relative border-2 border-dashed border-zinc-200 dark:border-zinc-800 hover:border-zinc-400 dark:hover:border-zinc-600 bg-zinc-50/50 dark:bg-zinc-950/40 rounded-3xl p-8 text-center cursor-pointer transition-all group"
              >
                <input
                  type="file"
                ...
                <div className="w-12 h-12 rounded-2xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300 flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform">
                  <Upload className="w-6 h-6" />
                </div>
                <p className="text-xs font-semibold font-mono text-zinc-800 dark:text-zinc-200">
              ...
              {isUploading && (
                <div className="space-y-2 p-3 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-100/80 dark:bg-zinc-900/80 font-mono text-xs">
              ...
              {file && (
                <div className="p-3 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 text-xs font-mono flex items-center justify-between text-zinc-700 dark:text-zinc-300">
              ...
              {previewUrl && (
                <div className="rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-black relative">
                  {activeTab === 'image' ? (
                    <img
                  ...
                  {result?.annotated_image_url && (
                    <div className="absolute top-2 right-2 px-2.5 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-[10px] font-mono font-bold flex items-center space-x-1 backdrop-blur-md">
            ...
            <CardContent className="space-y-6">
              ...
              {!loading && !result && !error && (
                <div className="py-20 text-center space-y-3">
                  ...
                  <p className="text-xs font-mono text-zinc-500">No media loaded</p>
                  <p className="text-[11px] text-zinc-400">Upload an image or video to view real-time compliance results.</p>
                </div>
              )}
              ...
              {result && !loading && (
                <div className="space-y-6">
                  ...
                  {/* Detected Tags */}
                  <div>
                    <h4 className="text-xs font-mono text-zinc-500 mb-3">DETECTION BREAKDOWN</h4>
                          ...
                          result.detections.map((d, i) => {
                            ...
                            return (
                              <Badge key={i} variant={isViolation ? 'warning' : 'success'}>
                                ...
                                <span>{d.class}</span>
                                <span className="ml-1 opacity-75">({Math.round(d.confidence * 100)}%)</span>
                              </Badge>
                            );
                          })
                        ) : (
                  ...
                  {/* AI Response */}
                  {result.answer && (
                    <div className="p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-100/80 dark:bg-zinc-900/80 space-y-2 text-xs">
                      ...
                      <p className="leading-relaxed font-sans text-zinc-700 dark:text-zinc-300">{result.answer}</p>
                    </div>
                  )}
                          ...
                          {result.evidence.map((item, idx) => (
                            <div key={idx} className="p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 text-xs space-y-1.5 font-mono">
                              ...
                              {item.section && item.section !== 'N/A' && (
                                <p className="text-[11px] text-zinc-400">{item.section}</p>
                              )}
                              <p className="text-[11px] leading-relaxed font-sans text-zinc-600 dark:text-zinc-300">{item.excerpt}</p>
                            </div>
                          ))}
                        </div>
                      )}
